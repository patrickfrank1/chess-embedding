...
def elo_filter(header: chess.pgn.Headers, white_elo_range: Tuple[int, int], black_elo_range: Tuple[int, int], debug=False) -> ct.GameFilter:
...
def time_control_filter(header: chess.pgn.Headers, time_range_minutes: Tuple[int, int], debug: bool = False) -> ct.GameFilter:
	logger = logging.getLogger(__name__)
	#headers are often non-standard, try..except!
	process_game = True
	try:
